  ...
  return (
    ...
    >
            ...
            <div className="flex flex-wrap gap-1.5 px-2">
              {(Object.entries(NOTE_COLORS) as [NoteColor, (typeof NOTE_COLORS)[NoteColor]][]).map(([key, c]) => (
                <button
                  ...
                  className={`w-5 h-5 rounded-full border-2 transition-all hover:scale-110 ${c.dot} ${
                    colorFilter.has(key)
      ...
      <div className="shrink-0 border-t px-1.5 py-1.5">
        <button
          onClick={() => onQuickFilterChange(quickFilter === 'trash' ? 'all' : 'trash')}
          className={`w-full flex items-center gap-2 px-2.5 py-2 text-sm rounded-md transition-colors ${
            quickFilter === 'trash'
              ...
              : 'text-ink-400 hover:text-muted-foreground hover:bg-muted'
          }`}
        >
