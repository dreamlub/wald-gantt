  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-background">
      ...
      <div className="h-12 border-b bg-card flex items-center px-4 gap-2 shrink-0">
        <span className="text-sm font-semibold text-foreground mr-2">메모장</span>
          ...
          <>
            {pinned.length > 0 && (
              <section className="mb-6">
                ...
                <NoteCollection notes={pinned} viewMode={viewMode} onUpdate={handleUpdate} onDelete={handleDelete} onOpen={setSelectedId} />
              </section>
            )}
            {regular.length > 0 && (
              <section>
                ...
                <NoteCollection notes={regular} viewMode={viewMode} onUpdate={handleUpdate} onDelete={handleDelete} onOpen={setSelectedId} />
              </section>
            )}
          </>
        )}
...
function NoteCollection({ notes, viewMode, onUpdate, onDelete, onOpen }: {
  notes: Note[]
  viewMode: ViewMode
  ...
  if (viewMode === 'list') {
    return (
      <div className="flex flex-col gap-1.5">
        {notes.map(note => (
          <NoteListItem key={note.id} note={note} onUpdate={onUpdate} onDelete={onDelete} onOpen={onOpen} />
        ))}
      </div>
    )
  }
    ...
    <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4 space-y-4">
      {notes.map(note => (
        <div key={note.id} className="break-inside-avoid">
          <NoteCard note={note} onUpdate={onUpdate} onDelete={onDelete} onOpen={onOpen} />
        </div>
      ))}
    </div>
  )
