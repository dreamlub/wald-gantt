...
import { IssueTrackerViewSwitch } from './issue-tracker-view-switch'
...
export function IssueTrackerBrandPanel({ brandId, stats }: Props) {
  if (brandId === 'all') {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-3 px-8 text-center">
        ...
        <p className="text-xs text-ink-400">Issue Tracker는 브랜드별로 확인할 수 있습니다.</p>
      </div>
    )
  }

  if (!stats) {
    return <IssueTrackerViewSwitch brandFilter={brandId} />
  }
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-5 px-8 text-center">
        ...
        <div>
          ...
          <p className="text-sm text-ink-400">Issue Tracker 생성 조건 미충족</p>
        </div>
        ...
        <p className="text-2xs text-ink-300 max-w-xs leading-relaxed">
          {stats.weekly_count === 0
            ...
            : '데이터가 더 쌓이면 /brand-timeline 스킬로 이슈를 생성할 수 있습니다.'}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-5 px-8 text-center">
      ...
      <div>
        <p className="text-base font-semibold text-ink mb-1">{brandId}</p>
        <p className="text-sm text-ink-400">아직 이슈가 생성되지 않았습니다</p>
      </div>
      <div className="bg-muted rounded-xl px-6 py-4 text-left space-y-3 w-full max-w-sm">
        <Stat icon={<TrendingUp size={13} />} label="위클리 리포트" value={`${stats.weekly_count}주`} ok need="" />
        <Stat icon={<Clock size={13} />} label="데일리 메시지" value={`${stats.daily_count}건`} ok need="" />
      </div>
      <p className="text-2xs text-ink-300 max-w-xs leading-relaxed">
        데이터가 충분합니다.{' '}
        <code className="bg-ink-100 px-1 py-0.5 rounded text-ink-500">/brand-timeline {brandId}</code>
        를 실행하면 이슈를 생성할 수 있습니다.
      </p>
    </div>
  )
}

function Stat({ icon, label, value, ok, need }: {
  icon: React.ReactNode; label: string; value: string; ok: boolean; need: string
}) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      {!ok && need && <span className="text-3xs text-ink-300">({need})</span>}
    </div>
  )
}
