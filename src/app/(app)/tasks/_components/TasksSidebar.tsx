...
export function TasksSidebar({
  ...
  const quickItems = [
    ...
    { key: 'overdue' as const,       label: '지연',          count: overdueCount,       icon: <span className="w-2 h-2 rounded-full bg-status-late shrink-0" /> },
    { key: 'start-delayed' as const, label: '시작 지연',     count: startDelayedCount,  icon: <span className="w-2 h-2 rounded-full bg-status-warn shrink-0" /> },
    ...
    { key: 'done' as const,          label: '완료',         count: doneCount,          icon: <span className="w-2 h-2 rounded-full bg-status-ok shrink-0" /> },
  ]
      ...
      <div className="flex flex-col gap-0.5 p-2 overflow-y-auto flex-1 min-h-0">
        ...
        {quickItems.map(item => (
          ...
          >
            ...
            <span className={`text-xs ${item.count > 0 && item.key === 'done' ? 'text-status-ok font-medium' : item.count > 0 && item.key !== 'all' ? 'text-status-late font-medium' : 'text-ink-400'}`}>
              {item.count}
            </span>
          </button>
        ))}
