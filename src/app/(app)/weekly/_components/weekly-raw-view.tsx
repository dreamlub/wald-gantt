...
function preprocessMd(content: string): string {
  const lines = content
    ...
    .replace(/\n{3,}/g, '\n\n')
    .replace(/==(.+?)==/g, '$1')
    // GFM 셀 내 <br> 누락으로 붙어버린 항목 분리 (heuristic)
    // ① 닫힘 괄호 직후 새 항목
    .replace(/\)(?=[가-힣0-9])/g, ')\n')
    // ② 한글 직후 날짜 숫자 (재현7월 → 재현\n7월)
    .replace(/([가-힣])([0-9]+[월일년])/g, '$1\n$2')
    // ③ N개 직후 한글 단어 (13개발트루스트 → 13개\n발트루스트, 3개월·3개년은 제외)
    .replace(/([0-9]+개)(?!월|년)([가-힣])/g, '$1\n$2')
    // ④ 한글 직후 라틴 대문자 약어 (변경PASS·완료B2B → \n 삽입)
    .replace(/([가-힣])([A-Z][A-Z0-9]+)/g, '$1\n$2')
    // ⑤ 상태어 직후 리스트 마커·한글 새 항목 (조사·어미 첫 음절이면 유지)
    .replace(/(완료|예정|필요|변경|오픈|업데이트|도입|확인|반영|제공)([\*\[]|[가-힣])/g,
      (_, w, next) => '이가는은를을에의도로으하되된됩고며서나와과만한'.includes(next) ? _ : w + '\n' + next
    )
    .split('\n')
